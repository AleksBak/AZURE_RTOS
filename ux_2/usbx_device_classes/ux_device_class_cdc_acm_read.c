		...
		ULONG requested_length, ULONG* actual_length)
{
	...
	if (device->ux_slave_device_state != UX_DEVICE_CONFIGURED)
	{
		...
		/* If trace is enabled, insert this event into the trace buffer. */
		UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_CONFIGURATION_HANDLE_UNKNOWN, device, 0, 0,
				UX_TRACE_ERRORS, 0, 0)
	...
	*actual_length = 0;
	...
	if (device->ux_slave_device_state != UX_DEVICE_CONFIGURED)
	{
		...
		/* If trace is enabled, insert this event into the trace buffer. */
		UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_TRANSFER_NO_ANSWER, transfer_request, 0, 0,
				UX_TRACE_ERRORS, 0, 0)
