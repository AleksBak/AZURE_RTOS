...
UINT _ux_device_stack_alternate_setting_set(ULONG interface_value, ULONG alternate_setting_value)
{
	...
	if (interface == UX_NULL)
	{
		...
		/* If trace is enabled, insert this event into the trace buffer. */
		UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_INTERFACE_HANDLE_UNKNOWN, interface, 0, 0,
				UX_TRACE_ERRORS, 0, 0)
	...
	while (device_framework_length != 0)
	{
		...
		if (descriptor_type == UX_CONFIGURATION_DESCRIPTOR_ITEM)
		{
					...
					== device->ux_slave_device_configuration_selected)
			{
				...
				/* We have found the configuration value that was selected by the host. We need to
				 * scan all the interface descriptors following this configuration descriptor
				 * and locate the interface for which the alternate setting must be changed. */
				while (device_framework_length != 0)
				{
					/* Get the length of the current descriptor. */
					descriptor_length = (ULONG)*device_framework;
					...
					if (descriptor_type == UX_INTERFACE_DESCRIPTOR_ITEM)
					{
								...
								interface_descriptor.bAlternateSetting == alternate_setting_value)
						{
							 ...
							 * we mount all the endpoints for this interface, we need to unmount
