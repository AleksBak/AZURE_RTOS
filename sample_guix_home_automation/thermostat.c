...
static VOID thermostat_controller_info_update(THERMOSTAT_INFO *info, int radial_angle)
{
    ...
    gx_prompt_text_set(&controller->thermostat_base_cold_heat_text, text);
...
static VOID thermostat_controller_reset(THERMOSTAT_BASE_CONTROL_BLOCK *widget)
{
    ...
    gx_numeric_prompt_value_set(&widget->thermostat_base_center_value, 0);
}
...
VOID thermostat_screen_init()
{
    ...
    gx_prompt_text_set(&thermostat_screen.base.screen_base_add_text, "ADD ROOM");
}