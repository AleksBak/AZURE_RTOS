		...
		UINT preempt_threshold, ULONG time_slice, UINT auto_start)
{
...
#ifdef TX_ENABLE_STACK_CHECKING
    /* Ensure that there are two ULONG of 0xEF patterns at the top and bottom of the thread's stack.
     * This will be used to check for stack overflow conditions during run-time. */
    stack_size = ((stack_size/(sizeof(ULONG))) * (sizeof(ULONG))) - (sizeof(ULONG));

    /* Ensure the starting stack address is evenly aligned. */
    new_stack_start = TX_POINTER_TO_ULONG_CONVERT(stack_start);
    updated_stack_start =  (
    		(((ULONG)new_stack_start) + ((sizeof(ULONG)) - ((ULONG)1)))
	...
	TX_TRACE_IN_LINE_INSERT(TX_TRACE_THREAD_CREATE, thread_ptr, priority,
			TX_POINTER_TO_ULONG_CONVERT(stack_start), stack_size, TX_TRACE_THREAD_EVENTS)
	...
	if (auto_start == TX_AUTO_START)
	{
		...
		if (TX_THREAD_GET_SYSTEM_STATE() >= TX_INITIALIZE_IN_PROGRESS)
		{
			...
			/* Pickup the current thread execute pointer, which corresponds to the highest
			 * priority thread ready to execute. Interrupt lockout is not required,
			 * since interrupts are assumed to be disabled during initialization. */
