...
/* Define the pointer that contains the system stack pointer. This is utilized when control returns
 * from a thread to the system to reset the current stack. This is setup in the low-level
 * initialization function. */
VOID* _tx_thread_system_stack_ptr;

/* Define the current thread pointer. This variable points to the currently executing thread.
 * If this variable is NULL, no thread is executing. */
TX_THREAD* _tx_thread_current_ptr;

/* Define the variable that holds the next thread to execute. It is important to remember
 * that this is not necessarily equal to the current thread pointer. */
...
/* Define the current state variable. When this value is 0, a thread is executing or the system
 * is idle. Other values indicate that interrupt or initialization processing is active. This
 * variable is initialized to TX_INITIALIZE_IN_PROGRESS to indicate initialization is active. */
volatile ULONG _tx_thread_system_state = TX_INITIALIZE_IN_PROGRESS;

/* Define the 32-bit priority bit-maps. There is one priority bit map for each 32 priority levels
 * supported. If only 32 priorities are supported there is only one bit map. Each bit within
 * a priority bit map represents that one or more threads at the associated thread priority are ready. */
