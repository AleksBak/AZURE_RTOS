...
static VOID on_passcode_input()
{
	...
	switch (input_code_count)
	{
		...
		case 3:
			...
			gx_prompt_text_set(&passcode_screen.passcode_screen_title, "Wrong Passcode");
			gx_prompt_text_color_set(&passcode_screen.passcode_screen_title, text_color_id,
...
static VOID on_clear()
{
	GX_ICON* icon;
	INT index = 0;
	INT icon_id = GX_PIXELMAP_ID_PASSCODE_ASTERIC;
	...
	gx_prompt_text_set(&passcode_screen.passcode_screen_title, "Enter Passcode");
	gx_prompt_text_color_set(&passcode_screen.passcode_screen_title, text_color_id, text_color_id,
			text_color_id);
}
