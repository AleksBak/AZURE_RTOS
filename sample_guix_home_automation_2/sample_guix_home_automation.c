...
#define CANVAS_SIZE			(DISPLAY_1_X_RESOLUTION * DISPLAY_1_Y_RESOLUTION)
...
TX_BYTE_POOL dynamic_pool;
__attribute__((section(".SD_RAM")))  GX_COLOR default_canvas_memory[CANVAS_SIZE];
__attribute__((section(".SD_RAM")))  GX_COLOR scratchpad[SCRATCHPAD_PIXELS];
...
UINT stm32f746_graphics_driver_setup_565rgb(GX_DISPLAY* graphic);
UINT stm32f746_graphics_driver_setup_24xrgb(GX_DISPLAY* graphic);
VOID demo_thread_entry(ULONG thread_input);
VOID touch_thread_entry(ULONG thread_input);
//#define GRAPHICS_DRIVER_SETUP_24XRGB stm32f746_graphics_driver_setup_24xrgb
//#define GRAPHICS_DRIVER_SETUP_565RGB stm32f746_graphics_driver_setup_565rgb
...
const GX_CHAR* day_names[7] = { "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday",
	"Friday", "Saturday" };

const GX_CHAR* month_names[12] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep",
	"Oct", "Nov", "Dec" };
...
{
	/* Create touch thread: thread_ptr, name_ptr, entry_function, entry_input, stack_start,
	...
	tx_thread_create(&touch_thread, "GUIX Touch Thread", touch_thread_entry, 0, touch_thread_stack,
			sizeof(touch_thread_stack), GX_SYSTEM_THREAD_PRIORITY - 1,
	 ...
	 * stack_size, priority, preempt_threshold, time_slice, auto_start */
	tx_thread_create(&demo_thread, "Demo Thread", demo_thread_entry, 0, demo_thread_stack,
			sizeof(demo_thread_stack), GX_SYSTEM_THREAD_PRIORITY + 1, GX_SYSTEM_THREAD_PRIORITY + 1,
...
VOID demo_thread_entry(ULONG thread_input)
{
	...
	gx_studio_display_configure(DISPLAY_1, stm32f746_graphics_driver_setup_24xrgb,
...
UINT main_screen_event_process(GX_WINDOW* window, GX_EVENT* event_ptr)
{
	switch (event_ptr->gx_event_type)
	{
		case GX_EVENT_SHOW:
			...
			gx_system_timer_start((GX_WIDGET* )window, CLOCK_TIMER, GX_TICKS_SECOND,
					GX_TICKS_SECOND);
...
static VOID decode_main_screen_jpeg()
{
	...
	gx_image_reader_create(&reader, map->gx_pixelmap_data, map->gx_pixelmap_data_size,
...
VOID custom_radial_slider_draw(GX_RADIAL_SLIDER* slider)
{
	...
	gx_context_pixelmap_get(slider->gx_radial_slider_info.gx_radial_slider_info_needle_pixelmap,
			&map);
	...
	if (map)
	{
		...
		gx_utility_rectangle_define(&dirty[0], xpos, ypos, xpos + map->gx_pixelmap_width - 1,
				ypos + map->gx_pixelmap_height - 1);
