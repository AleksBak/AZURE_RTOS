...
{
    GX_CHAR *enable_disable_text;

    if (lock_info.lock_enabled)
    {
        enable_disable_text = "ENABLED";
    }
    else
    {
        enable_disable_text = "DISABLED";
    }

    gx_prompt_text_set(&locks_page_1.locks_page_1_enable_disable_text, enable_disable_text);
}
...
static VOID lock_unlock_one_door(LOCKS_DOOR_BASE_CONTROL_BLOCK *base, GX_BOOL lock)
{
    ...
    while (child)
    {
        ...
        case GX_TYPE_PROMPT:
            ...
            gx_prompt_text_set((GX_PROMPT *)child, lock_text);
...
VOID locks_screen_init()
{
    ...
    gx_prompt_text_set(&locks_screen.base.screen_base_add_text, "ADD LOCK");
}
