...
/* Defined, this value represents the maximum number of bytes received on a control endpoint in the
 * device stack. The default is 256 bytes but can be reduced in memory constraint environments. */
 ...
 * on any endpoint. This value cannot be less than the maximum packet size of any endpoint.
 * The default is 4096 bytes but can be reduced in memory constraint environments. For CD-ROM
 * support in the storage class, this value cannot be less than 2048. */
...
/* Defined, this value represents the maximum number of Ed, regular TDs and Isochronous TDs. These
 * values depend on the type of host controller and can be reduced in memory constraint environments. */
#define UX_MAX_ED                                           80
#define UX_MAX_TD                                           128
#define UX_MAX_ISO_TD                                       1

/* Defined, this value represents the maximum size of the HID decompressed buffer. This cannot be
 * determined in advance so we allocate a big block, usually 4K but for simple HID devices like
 * keyboard and mouse it can be reduced a lot. */
 ...
 * (a HID report from the device is received whenever there is a change in a key state i.e. when a
 * key is pressed or released. The report contains every key that is down). There are limitations to
 * this method such as not being able to determine when a key has been released.
 *
 * Defined, this value causes ux_host_class_hid_keyboard_key_get to only report key changes i.e. key
 ...
 *
 * Defined, this value causes ux_host_class_hid_keyboard_key_get to only report key pressed/down
 * changes; key released/up changes are not reported. */
...
/* Defined, this value represents the the maximum length of HID reports on the device. */
...
/* Defined, this value will include the OTG polling thread. OTG can only be active if both
 * host/device are present. */
