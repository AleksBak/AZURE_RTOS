...
UCHAR                 *setup_request;
UX_HCD_STM32_ED     *ed;
UX_HCD_STM32_TD     *setup_td;
UX_HCD_STM32_TD     *chain_td;
UX_HCD_STM32_TD     *data_td;
UX_HCD_STM32_TD     *tail_td;
UX_HCD_STM32_TD     *status_td;
    ...
    *(setup_request + UX_SETUP_REQUEST) =       transfer_request -> ux_transfer_request_function;
    ...
    while (transfer_request_payload_length != 0)
    {
        ...
        if (transfer_request_payload_length > ed -> ux_stm32_ed_endpoint -> ux_endpoint_descriptor.wMaxPacketSize)
            ...
            control_packet_payload_length =  ed -> ux_stm32_ed_endpoint -> ux_endpoint_descriptor.wMaxPacketSize;
        else
    ...
    /* Now, program the status phase.  Get a TD.  */
    ...
    if (status_td == UX_NULL)
    {
        ...
        /* Could not get a TD.  Free up resources.  */
        _ux_utility_memory_free(setup_request);

        /* Parse the data TDs and free them.  */
        if (data_td != UX_NULL)
        {
    ...
    if ((transfer_request -> ux_transfer_request_type&UX_REQUEST_DIRECTION) == UX_REQUEST_IN)
        ...
        status_td -> ux_stm32_td_direction =  UX_HCD_STM32_TD_OUT;
    else
