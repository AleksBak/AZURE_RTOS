...
#define UX_BOS_DEVICE_CAPABILITY_DESCRIPTOR_ITEM                        0x0A
#define UX_INTERFACE_ASSOCIATION_DESCRIPTOR_ITEM                        0x0B
#define UX_BOS_DESCRIPTOR_ITEM                                          0x0F
...
/* Define USBX Error Code constants. The following format describes their meaning:
...
/* Define USBX Endpoint Container structure (used only in host source). */
...
{
	/* Handle of the device. This is typically the address of the instance
	 * of this structure for the device. */
	ULONG ux_device_handle;

	/* Obsolete value. Unused. */
	ULONG ux_device_type;

	/* Device State, which can have one of the following values:
	 * UX_DEVICE_RESET					0
	...
	/* Address of the device after the SET_ADDRESS command has been accepted (from 1 to 127). */
	...
	/* Index of the port of the parent device (root hub or hub). */
	ULONG ux_device_port_location;

	/* Maximum power in mA that the device may take in the selected configuration. */
	...
	/* Index of the current configuration being used by this device. */
	UINT ux_device_current_configuration;

	TX_SEMAPHORE ux_device_protection_semaphore;

	/* Device container pointer of the parent of this device.
	 * If the pointer is null, the parent is the root hub of the controller. */
	struct UX_DEVICE_STRUCT* ux_device_parent;

	/* Pointer to the class type that owns this device. */
	struct UX_HOST_CLASS_STRUCT* ux_device_class;

	/* Pointer to the instance of the class that owns this device. */
	VOID* ux_device_class_instance;

	/* USB Host Controller Instance where this device is attached. */
	struct UX_HCD_STRUCT* ux_device_hcd;

	/* Pointer to the first configuration container for this device. */
	struct UX_CONFIGURATION_STRUCT* ux_device_first_configuration;

	/* Pointer to the next device in the list of device on any of the buses detected by USBX. */
	struct UX_DEVICE_STRUCT* ux_device_next_device;

	/* USB device descriptor. */
	struct UX_DEVICE_DESCRIPTOR_STRUCT ux_device_descriptor;

	/* Descriptor of the default control endpoint used by this device. */
	struct UX_ENDPOINT_STRUCT ux_device_control_endpoint;

	/* Array of Hub TTs for the device */
...
/* Define USBX Device Controller Endpoint structure. (used only in device source) */
...
typedef struct UX_SLAVE_DEVICE_STRUCT
{
	...
	struct UX_CONFIGURATION_DESCRIPTOR_STRUCT ux_slave_device_configuration_descriptor;
...
typedef struct UX_SYSTEM_STRUCT
{
	...
	VOID (*ux_system_error_callback_function)(UINT system_level, UINT system_context,
			UINT error_code);
} UX_SYSTEM;
...
typedef struct UX_SYSTEM_SLAVE_STRUCT
{
	...
	UINT (*ux_system_slave_device_vendor_request_function)(ULONG, ULONG, ULONG, ULONG, UCHAR*,
			ULONG*);
} UX_SYSTEM_SLAVE;
...
UINT ux_host_stack_class_instance_get(UX_HOST_CLASS* host_class, UINT class_index,
		VOID** class_instance);
...
UINT ux_host_stack_device_configuration_get(UX_DEVICE* device, UINT configuration_index,
...
UINT ux_host_stack_hcd_register(UCHAR* hcd_name,
		UINT (*hcd_initialize_function)(struct UX_HCD_STRUCT*), ULONG hcd_param1, ULONG hcd_param2);
UINT ux_host_stack_initialize(UINT (*ux_system_host_change_function)(ULONG, UX_HOST_CLASS*, VOID*));
UINT ux_host_stack_interface_endpoint_get(UX_INTERFACE* interface, UINT endpoint_index,
...
UINT ux_device_stack_transfer_request(UX_SLAVE_TRANSFER* transfer_request, ULONG slave_length,
		ULONG host_length);
