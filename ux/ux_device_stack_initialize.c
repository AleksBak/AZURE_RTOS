...
UX_COMPILE_TIME_ASSERT(
		!UX_OVERFLOW_CHECK_MULC_ULONG(sizeof(UX_SLAVE_CLASS), UX_MAX_SLAVE_CLASS_DRIVER),
...
/*    (ux_system_slave_change_function)     Function to be called when    */
/*                                          the device state changes.     */
/*                                          The application can request a */
/*                                          call back when the controller */
/*                                          changes its state. The two    */
/*                                          main states for the           */
/*                                          controller are:               */
/*                                             UX_DEVICE_SUSPENDED        */
/*                                             UX_DEVICE_RESUMED          */
/*                                          If the application does not   */
/*                                          need Suspend/Resume signals,  */
/*                                          it would supply a UX_NULL     */
/*                                          function.                     */
/*                                                                        */
/*  OUTPUT                                                                */
/*                                                                        */
		...
		ULONG language_id_framework_length, UINT (*ux_system_slave_change_function)(ULONG))
{
	...
	/* Allocate some memory for the Control Endpoint. First get the address
	 * of the transfer request for the control endpoint. */
	...
	if (status == UX_SUCCESS)
	{
		...
		if (interfaces_found == 0)
		{
			...
			/* If trace is enabled, insert this event into the trace buffer. */
			UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_DESCRIPTOR_CORRUPTED, device_framework, 0, 0,
					UX_TRACE_ERRORS, 0, 0)
	...
	if (status == UX_SUCCESS)
	{
		...
		UX_SLAVE_INTERFACE* interfaces_pool = _ux_utility_memory_allocate_mulc_safe(
				UX_NO_ALIGN, UX_REGULAR_MEMORY, interfaces_found, sizeof(UX_SLAVE_INTERFACE));
	...
	if (endpoints_found != 0 && status == UX_SUCCESS)
	{
		...
		endpoints_pool = _ux_utility_memory_allocate_mulc_safe(UX_NO_ALIGN, UX_REGULAR_MEMORY,
				endpoints_found, sizeof(UX_SLAVE_ENDPOINT));
		...
		else
		{
			 ...
			 * maximum buffer size. We also assign the semaphore used by the endpoint to synchronize
			 * transfer completion. */
